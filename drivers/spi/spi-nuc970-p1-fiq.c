...
static int nuc970_spi1_probe(struct platform_device *pdev)
{
 ...
 #elif defined(CONFIG_SPI_NUC970_P1_DO_ONLY) && defined(CONFIG_SPI_NUC970_P1_PB)
	p = devm_pinctrl_get_select(&pdev->dev, "spi1-do-only-PB");
 #elif defined(CONFIG_SPI_NUC970_P1_DO_ONLY) && defined(CONFIG_SPI_NUC970_P1_PI)
	p = devm_pinctrl_get_select(&pdev->dev, "spi1-do-only-PI");
 #endif
#endif
	if(IS_ERR(p)) {
